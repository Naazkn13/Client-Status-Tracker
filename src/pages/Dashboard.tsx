...
import { Search, RefreshCw, Filter, ChevronUp, ChevronDown, Clock, Pencil, FileText, X, UserPlus, LogOut } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleClientSave = async (data: { name: string; industry: string }, id?: string) => {
    if (id) {
      ...
      const { data: inserted, error } = await supabase
        .from('clients')
        .insert({ ...data, row_number: nextRow, status_color: 'yellow', status_note: '', detailed_notes: '', updated_by: user?.email || 'Unknown' })
        .select()
        .single();
      if (error) throw error;
  ...
  const handleSave = async (
    ...
    clientFields.updated_by = user?.email || 'Unknown';
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={signOut}
              className="flex items-center gap-2 px-3 py-2 text-gray-500 hover:text-gray-900 text-sm font-medium transition-colors"
            >
              <LogOut size={14} />
              Sign Out
            </button>
